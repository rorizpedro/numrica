    ...
    <>
      <div style={{ background: '#f8f9fb', minHeight: '100vh' }}>
        ...
        <section style={{ maxWidth: 840, margin: '0 auto', padding: '28px 24px 24px' }}>
          ...
          {r ? (
            <div style={{ marginTop: 16 }}>
              ...
              {/* Share */}
              {(() => {
                const totalDebt = debtInputs.reduce((s, d) => s + d.balance, 0)
                const pageUrl   = typeof window !== 'undefined' ? window.location.href : 'https://numrica.com/debt-payoff'
                const shareText = `Paying off ${fmt(totalDebt)} in debt by ${monthsToDate(r.months)} with the ${thisLabel} method — only ${fmt(r.totalInterest)} in interest. Check my plan:`
                ...
                const btnBase: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, borderRadius: 8, padding: '7px 13px', cursor: 'pointer', textDecoration: 'none', border: 'none' }
                return (
                  <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    <button onClick={copyShareUrl} style={{ ...btnBase, color: copied ? '#22c55e' : '#6b7280', background: '#f3f4f6', border: '1px solid #e5e7eb' }}>
                      <Share2 size={13} />
                      {copied ? 'Copied!' : 'Copy link'}
                    </button>
                    <a href={twitterHref} target="_blank" rel="noopener noreferrer" style={{ ...btnBase, color: '#fff', background: '#000' }}>
                      <span style={{ fontWeight: 800, fontSize: 13, lineHeight: 1 }}>𝕏</span> Share
                    </a>
                    <a href={whatsappHref} target="_blank" rel="noopener noreferrer" style={{ ...btnBase, color: '#fff', background: '#25d366' }}>
        ...
        <div className="embed-hide" style={{ maxWidth: 840, margin: '0 auto', padding: '4px 24px', display: 'flex', justifyContent: 'flex-end' }}>
          <EmbedButton tool="debt-payoff" title="Debt Payoff Planner" />
        </div>
